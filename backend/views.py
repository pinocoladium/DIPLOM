...
from backend.auth import hash_password, generate_password
from backend.serializers import ClientSerializer, ProductSerializer, ShopSerializer
    ...
    def get(self, request):
        if request.user.is_authenticated:
            if request.user.type == 'shop':
                if request.user.is_authenticated:
                    serializers = ShopSerializer(Shop.objects.get(client=request.user.id))
                    return Response(serializers.data)
        ...
        if {'name'}.issubset(request.data):
            if request.user.type != 'shop':
                ...
                if shop_serializer.is_valid():
                    ...
                    shop.save()
                    return Response({'status': True, 'name shop': shop.name, 'state': shop.state})
                return Response({'Status': False, 'Errors': shop_serializer.errors})
            return Response({'Status': False, 'Errors': 'Нельзя создать более 1 магазина на аккаунт'})
        return Response({'Status': False, 'Errors': 'Не указаны все необходимые аргументы'})
